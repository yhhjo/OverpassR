...
server <- function(input, output, session){
  ...
  generateS2 <- function(lon, lat){
    ...
    times <- strftime(swaths$begin, format = "%H:%M:%S", tz = "UTC") %>% hms()
    datetimes <- data.frame("Datetimes (UTC)" = NULL)
    ...
    for(i in 1:length(dates)) {
      ...
      x <- data.frame("Datetimes (UTC)" = x[x <= stop])
      datetimes <- rbind(datetimes, x)
    }
    
    output <- datetimes %>% as.character.Date() %>% merge(mgrs$Name)
    names(output) <- c("Datetimes (UTC)","MGRS")
    cbind(output, "Path" = NA, "Row" = NA, "Lat" = lat, "Lon" = lon, "Satellite" = "Sentinel2") %>% unique() %>% update_output()
  ...
  generate <- function(lon, lat, ref){
    ...
    for(r in 1:length(paths)){
      ...
      updating_table <- rbind(updating_table, cbind("Datetimes (UTC)" = dates,"Path" = paths[r], "Row" = rows[r], "MGRS" = NA,
